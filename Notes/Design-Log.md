...
| R2 | **Shed roof on the on-end pockets** | R1's fix was wrong in the same way Q was, one step removed: a +X teardrop whose bottom-face side grew out of the open pocket — a 45°-shaped cantilever. The face-angle audit passed it. David's markup on the Q section: continue the vee-side 45° tangent line straight up through the jaw bottom, one plane, no crown. That grows off the solid vee side layer by layer, which is the direction that is actually supported. New check, `printcheck.py`, slices every layer and measures material with nothing under it: Q 733 mm², R1 517, R2 90 (all of it the 9/32 hole's 1.8 mm bridge). Far pocket's plane runs out the jaw end 0.21 in up; cosmetic, off the ledge. |
...
| clamshell A | **Reaction-rod clamshell**, draft | David: a keel-like clamshell holding a Geissele Reaction Rod, flats up inside the bore, rubber-band preload in self-supporting double-45° grooves, keep 4.5 in. Rod 0.995 OD / 0.745 flats / 11.25 long. Parting plane vertical so the vise closes it; flats face the jaws; 0.030 gap; grooves 0.08 deep stepped one layer (0.14 ate the flange); pins loose in both halves (printed-on pins are horizontal overhangs on end). 3.50 not 4.50: the receiver can't overlap the jaws, so shell = rod behind the receiver with lugs seated − margin, ≈3.5 on assumed numbers. Unprinted; Rev B on the measurement. |
| insert v5 | **Sloped magwell seat kept under the stub** | David: "mag insert was a bit too short to engage mag catch." v4 cut the source block at z = 52, above what I had called the tang/slant transition. The slant is the magwell seat: the block's shoulder slopes 13 mm over the mag's length (11°, the rake of the magwell mouth) so the lower sits flat with the stub fully home. Standing the bare stub on a flat base left the raked mouth touching at one end with the stub ~1/2 in short, and the catch notch (30–36 mm above the stub bottom) never reached the catch. v5 cuts at z = 39 and keeps the seat; notch band 2.1–2.3 in above the pocket floor, height 2.92 → 3.43. The source STL is two overlapping shells (block + full mag body); a straight union re-merged into non-manifold edges, so the seat is rebuilt parametrically (mag cut clean at 52 on a prism of its own outline down to 39, plus the wedge) and the mag mesh decimated to 30k faces. Watertight, one body, printcheck 4 mm². |
...
6. **Keel plate.** `Doyle keel v9 - claude PET-CF17.3mf` built 2026-09-10 straight from the PA612 project
   ...
   270–300 °C, bed 70–80, fan 0, door open. Re-sliced 2026-09-10 in Orca with insert v5 added:
   4 h 55 m, 259 g (83 m). Process saved as "… claude keel PET-CF".
7. **Clamshell Rev B.** Measure, with the barrel-extension lugs seated: rod exposed behind the receiver's rear
